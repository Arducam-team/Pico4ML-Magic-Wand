...
constexpr int stroke_transmit_stride = 2;       // 笔画传递步幅
constexpr int stroke_transmit_max_length = 160; // 笔画传输最大长度
constexpr int stroke_max_length =
    stroke_transmit_max_length * stroke_transmit_stride; // 笔画最长长度
constexpr int stroke_points_byte_count =
    2 * sizeof(int8_t) * stroke_transmit_max_length; // 笔画点字节数
constexpr int stroke_struct_byte_count =
    (2 * sizeof(int32_t)) + stroke_points_byte_count; // 笔画结构字节数
constexpr int moving_sample_count = 50;               // 移动样本数

float current_velocity[3] = {0.0f, 0.0f, 0.0f};        // 当前速度
float current_gravity[3] = {0.0f, 0.0f, 0.0f};         // 当前引力
float current_gyroscope_drift[3] = {0.0f, 0.0f, 0.0f}; // 当前陀螺仪漂移

int32_t stroke_length = 0; // 笔画长度
uint8_t stroke_struct_buffer[stroke_struct_byte_count] = {}; // 笔画结构缓冲区
int32_t *stroke_state =
    reinterpret_cast<int32_t *>(stroke_struct_buffer); // 笔锋状态
int32_t *stroke_transmit_length =
    reinterpret_cast<int32_t *>(stroke_struct_buffer + sizeof(int32_t)); // 笔画传输长度
int8_t *stroke_points =
    reinterpret_cast<int8_t *>(stroke_struct_buffer + (sizeof(int32_t) * 2)); // 笔点
...
__attribute__((unused)) float acceleration_sample_rate = 0.0f;
...
__attribute__((unused)) int ReadGyroscope() {
  // Keep track of whether we stored any new data
  int new_samples = 0;
  // Loop through new samples and add to buffer
  while (ICM20948::dataReady()) {
...
// 归一化向量
__attribute__((unused)) void NormalizeVector(const float *in_vec, float *out_vec) {
...
void EstimateGravityDirection(float *gravity) {
  ...
  gravity[0] = x_total / (float )samples_to_average;
  gravity[1] = y_total / (float )samples_to_average;
  gravity[2] = z_total / (float )samples_to_average;
}

// 更新速度
void UpdateVelocity(int new_samples, const float *gravity) {
...
void EstimateGyroscopeDrift(float *drift) {
  ...
  drift[0] = x_total / (float )samples_to_average;
  drift[1] = y_total / (float )samples_to_average;
  drift[2] = z_total / (float )samples_to_average;
}

// 更新方向
void UpdateOrientation(int new_samples, float *gravity, const float *drift) {
  // update the current orientation by integrating the angular velocity over
  // time
...
bool IsMoving(int samples_before) {
  ...
  // Note: this is different from how we calulate isMoving in
  ...
  *done_just_triggered = false;
  ...
  for (int i = 0; i < new_samples; ++i) {
    ...
    *done_just_triggered = ((old_state != eDone) && (*stroke_state == eDone));
    ...
    const float x_mean = x_total / (float )stroke_length;
    const float y_mean = y_total / (float )stroke_length;
    const float z_mean = z_total / (float )stroke_length;
    ...
    *stroke_transmit_length = stroke_length / stroke_transmit_stride;
    ...
    for (int j = 0; j < *stroke_transmit_length; ++j) {
      ...
      auto unchecked_x = static_cast<int32_t>(roundf(x_axis * 128.0f));
      int8_t stored_x;
      if (unchecked_x > 127) {
        stored_x = 127;
        ...
        stored_x = (int8_t )unchecked_x;
      }
      stroke_entry[0] = stored_x;

      auto unchecked_y = static_cast<int32_t>(roundf(y_axis * 128.0f));
      int8_t stored_y;
      if (unchecked_y > 127) {
        stored_y = 127;
